...
void GdsWriter::write(::GdsParser::GdsWriter& gw, GdsPolygon const& object) const
{
	...
	for(int i = 0; i < vx.size() - 1; ++i){
		ps.emplace_back(vx[i], vy[i]);
	}
	PolygonsInGDS::ShapeInGDS* shape = new PolygonsInGDS::ShapeInGDS(ps, object.layer(), object.datatype());

	polygons_.shapes.emplace_back(shape);
}
